...
ruta_log = 'E02'
carpeta_datos = 'E01/precip.MIROC5.RCP60.2006-2100.SDSM_REJ'
archivo_log = os.path.join(ruta_log, 'datos.log')
archivo_csv = os.path.join(ruta_log, 'estadisticas.csv')

# Crear directorio si no existe
os.makedirs(ruta_log, exist_ok=True)

# Función para escribir en log con timestamp
def escribir_log(contenido, archivo=archivo_log, modo='a'):
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    with open(archivo, modo, encoding='utf-8') as f:
        f.write(f"Fecha de entrada: {timestamp}\n")
        f.write(contenido + "\n\n")
...
def procesar_estadisticas():
    # (Mantener el código de estadísticas original aquí)
    # ... [El código de estadísticas completo] ...
    ...
    # Devolver datos necesarios para CSV y gráficos
    return {
        'datos_globales': datos_globales,
        'medias_anuales_por_anio': medias_anuales_por_anio,
        'promedio_pasados': promedio_pasados,
        'promedio_futuros': promedio_futuros,
        'dia_mas_lluvioso_pasado': dia_mas_lluvioso_pasado,
        'dia_menos_lluvioso_pasado': dia_menos_lluvioso_pasado,
        'dia_mas_lluvioso_futuro': dia_mas_lluvioso_futuro,
        'dia_menos_lluvioso_futuro': dia_menos_lluvioso_futuro
    }

# Función para generar CSV
def generar_csv(datos_globales, medias_anuales, promedio_pasados):
    with open(archivo_csv, 'w', newline='', encoding='utf-8') as f:
        ...
        for anio in sorted(datos_globales):
            ...
            media = medias_anuales.get(anio, 0)
            
            # Calcular tasa de variación
            if anio_anterior and total_anterior and total_anterior != 0:
                tasa = ((total - total_anterior) / total_anterior) * 100
            else:
                tasa = None
                
            # Determinar clasificación
            umbral = promedio_pasados if anio <= 2024 else promedio_futuros
            clasificacion = 'Pluvioso' if total > umbral else 'Seco'
            
            writer.writerow([
                anio,
                int(total),
                round(media, 2),
                round(tasa, 2) if tasa is not None else 'N/A',
                clasificacion
            ])
...
def generar_graficos(datos_globales, medias_anuales):
    años = sorted(datos_globales.keys())
    totales = [datos_globales[anio]['total'] for anio in años]
    medias = [medias_anuales[anio] for anio in años]
    
    # Gráfico de barras (Totales anuales)
    plt.figure(figsize=(15, 6))
    plt.bar(años, totales)
    plt.title('Precipitación Total Anual')
    plt.xlabel('Año')
    plt.ylabel('L/m²')
    plt.savefig(os.path.join(ruta_log, 'total_anual_barras.png'))
    plt.close()
    
    # Gráfico de líneas (Medias anuales)
    plt.figure(figsize=(15, 6))
    plt.plot(años, medias)
    plt.title('Evolución de la Media Anual')
    plt.xlabel('Año')
    plt.ylabel('L/m²')
    plt.grid(True)
    plt.savefig(os.path.join(ruta_log, 'media_anual_lineas.png'))
    plt.close()
    
    # Gráfico de dispersión (Variación)
    tasas = []
    for i in range(1, len(totales)):
        tasa = ((totales[i] - totales[i-1]) / totales[i-1]) * 100 if totales[i-1] != 0 else 0
        tasas.append(tasa)
    
    plt.figure(figsize=(15, 6))
    plt.scatter(años[1:], tasas)
    plt.title('Tasas de Variación Interanual')
    plt.xlabel('Año')
    plt.ylabel('Variación (%)')
    plt.grid(True)
    plt.savefig(os.path.join(ruta_log, 'variacion_dispersion.png'))
    plt.close()
...
if __name__ == "__main__":
    ...
    # Paso 2: Procesar estadísticas
    resultados = procesar_estadisticas()
    
    # Paso 3: Generar CSV
    generar_csv(resultados['datos_globales'], 
               resultados['medias_anuales_por_anio'],
               resultados['promedio_pasados'])
    
    # Paso 4: Generar gráficos
    generar_graficos(resultados['datos_globales'],
                    resultados['medias_anuales_por_anio'])
    
    print("Proceso completo. Resultados guardados en:", ruta_log)